...
int main() {
    int n = 7;
    //图像灰度数组{p1,p2,p3,p4,p5,p6}
    //下标从1开始计数
    int p[] = {0, 0b1010, 0b1100, 0b1111, 0b11111111, 0b11, 0b01};
    int s[n], l[n], b[n];
...
void Traceback(int n, int &i, int s[], int l[]) {
    if (n == 0) return;
    Traceback(n - l[n], i, s,
              l);  // p1,p2,p3,...,p(n-l[n])像素序列，最后一段有l[n]个像素
    s[i++] =
        n - l[n];  //重新为s[]数组赋值，用来存储分段位置，最终i为共分了多少段
}
...
void Output(int s[], int l[], int b[], int n) {
    ...
    int m = 0;
    Traceback(n, m, s,
              l);  // s[0]:第一段从哪分，s[1]:第二段从哪分...，s[m-1]第m段从哪分
    ...
    for (int j = 1; j <= m; j++) {
        ...
        // 此处b[j]有误，弃之
        // DONE: Check this out with my teacher
        cout << "段长度：" << l[j] << endl;
    }
}
